<script>
	...
	async function loadFormData() {
		...
		try {
			...
			pageError =
				"We could not load the invoice details. Please refresh and try again.";
		} finally {
			isLoading = false;
		}
	}
...
<main class="page-shell">
	<header class="page-header">
		<div>
			...
			<p class="subtitle">
				Record a new barrel rental and customer details.
			</p>
		</div>
	...
	>
		...
		<section class="card">
			<div class="card-heading">
				...
				<label>
					...
					<select class="control" bind:value={vehicleId}>
			...
			<div class="form-grid">
				<label class="full-width">
					...
					<select bind:value={barrelId} disabled={isLoading} required>
						<option value=""
							>{isLoading
								? "Loading available barrels…"
								: "Select an available barrel"}</option
						>
						{#each availableBarrels as barrel (barrel.id)}
							<option value={barrel.id}
								>{barrel.code}{barrel.type
									? ` · ${barrel.type}`
									: ""}</option
							>
						{/each}
				...
				<label class="full-width">
					...
					<textarea
						rows="3"
						placeholder="Add notes about the barrel or rental"
		...
		<div class="actions">
			<button
				class="secondary"
				type="button"
				onclick={() => goto("/invoices")}>Cancel</button
			>
			<button
				class="primary"
				type="submit"
...
<style>
	...
	.invoice-number {
		...
		background: #fff;
		text-align: right;
		box-shadow: 0 3px 12px rgb(30 52 90 / 5%);
	}
	.invoice-number span {
		display: block;
		margin-bottom: 4px;
	...
	.card {
		...
		border-radius: 14px;
		background: #fff;
		box-shadow: 0 5px 22px rgb(30 52 90 / 5%);
	}
	.card-heading {
		display: flex;
		align-items: center;
	...
	.primary {
		...
		background: #3563e9;
		color: #fff;
		box-shadow: 0 4px 10px rgb(53 99 233 / 22%);
	}
